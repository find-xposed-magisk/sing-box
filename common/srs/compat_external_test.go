...
func TestCompatWithSingBox117(t *testing.T) {
	...
	verifyNewBinaryReadsOldRuleSet(t, oldBinary, workDirectory)
}

func verifyOldBinaryReadsNewRuleSet(t *testing.T, oldBinary, workDirectory string) {
	t.Helper()
	rule, logicalRule, ruleSet := buildBoundaryRuleSet()
...
func verifyOldBinaryMatchesAdGuardRuleSet(t *testing.T, oldBinary, workDirectory string) {
	...
	rule := option.HeadlessRule{
		...
		DefaultOptions: option.DefaultHeadlessRule{
			AdGuardDomain: badoption.Listable[string]{
				"||ads.example.com^",
				"||tracker.example.org^",
	...
	for _, domain := range []string{"ads.example.com", "tracker.example.org", "analytics.example.net"} {
		output, err := runCommandOutput(workDirectory, oldBinary, "rule-set", "match", "-f", "binary", outputPath, domain)
		if err != nil {
			t.Fatalf("match adguard rule-set: %v\n%s", err, output)
		}
		if !strings.Contains(string(output), "match rules.[0]") {
			t.Fatalf("expected adguard match output for %s, got: %s", domain, output)
		}
	}
}

func verifyNewBinaryReadsOldRuleSet(t *testing.T, oldBinary, workDirectory string) {
	t.Helper()
	rule, logicalRule, ruleSet := buildBoundaryRuleSet()
	jsonPath := filepath.Join(workDirectory, "boundary.json")
	writeRuleSetJSON(t, jsonPath, option.PlainRuleSetCompat{
		Version: C.RuleSetVersion3,
		Options: ruleSet,
	})
	runCommand(t, workDirectory, oldBinary, "rule-set", "compile", filepath.Base(jsonPath))
	compiledPath := filepath.Join(workDirectory, "boundary.srs")
	compat := readRuleSet(t, compiledPath)
	if len(compat.Options.Rules) != 2 {
		t.Fatalf("unexpected rules length: %d", len(compat.Options.Rules))
	}
	assertDefaultRuleMatches(t, rule.DefaultOptions, compat.Options.Rules[0].DefaultOptions)
...
func buildBoundaryRuleSet() (option.HeadlessRule, option.HeadlessRule, option.PlainRuleSet) {
	rule := option.HeadlessRule{
		Type: C.RuleTypeDefault,
		DefaultOptions: option.DefaultHeadlessRule{
			QueryType: badoption.Listable[option.DNSQueryType]{
				option.DNSQueryType(1),
				option.DNSQueryType(15),
				option.DNSQueryType(28),
				option.DNSQueryType(65),
			},
			Network:       badoption.Listable[string]{"tcp", "udp"},
			Domain:        badoption.Listable[string]{"example.com", "example.org", "long.example"},
			DomainSuffix:  badoption.Listable[string]{".example.net", ".sub.example.org"},
			DomainKeyword: badoption.Listable[string]{"keyword", "keyword-extended"},
			DomainRegex:   badoption.Listable[string]{`^.*\.regex\.test$`, `^test[0-9]+\.example\.com$`},
			SourceIPCIDR:  badoption.Listable[string]{"10.0.0.0/8", "fd00::/8"},
			IPCIDR: badoption.Listable[string]{
				"1.2.3.0/24",
				"2001:db8::/32",
				"198.51.100.0/24",
			},
			SourcePort:      badoption.Listable[uint16]{53, 5353},
			SourcePortRange: badoption.Listable[string]{"1000-2000", "2001-3000"},
			Port:            badoption.Listable[uint16]{80, 443, 8080},
			PortRange:       badoption.Listable[string]{"3000-4000", "4001-5000"},
			ProcessName:     badoption.Listable[string]{"proc", "daemon"},
			ProcessPath:     badoption.Listable[string]{"/usr/bin/proc", "/opt/app/bin/proc"},
			ProcessPathRegex: badoption.Listable[string]{
				`^/usr/bin/.*$`,
				`^/opt/.+`,
			},
			PackageName:          badoption.Listable[string]{"com.example.app", "org.example.app"},
			WIFISSID:             badoption.Listable[string]{"TestWiFi", "Cafe-WiFi"},
			WIFIBSSID:            badoption.Listable[string]{"00:11:22:33:44:55", "66:77:88:99:aa:bb"},
			NetworkType:          badoption.Listable[option.InterfaceType]{option.InterfaceType(C.InterfaceTypeWIFI), option.InterfaceType(C.InterfaceTypeCellular), option.InterfaceType(C.InterfaceTypeEthernet), option.InterfaceType(C.InterfaceTypeOther)},
			...
			Rules: []option.HeadlessRule{
				{
					Type: C.RuleTypeDefault,
					DefaultOptions: option.DefaultHeadlessRule{
						Domain: badoption.Listable[string]{"logic.example.com", "logic.example.org"},
					},
				},
				{
					Type: C.RuleTypeDefault,
					DefaultOptions: option.DefaultHeadlessRule{
						IPCIDR: badoption.Listable[string]{"203.0.113.0/24", "2001:db8:1::/48"},
					},
				},
				{
					Type: C.RuleTypeDefault,
	...
	return rule, logicalRule, ruleSet
}
...
func assertRuleSetHasAdGuard(t *testing.T, path string) {
	...
	for _, needle := range []string{"ads.example.com", "tracker.example.org"} {
		if !hasRule(ruleSet.Options.Rules, func(rule option.DefaultHeadlessRule) bool {
			return containsSubstring(rule.AdGuardDomain, needle)
		}) {
...
func writeRuleSetJSON(t *testing.T, path string, ruleSet option.PlainRuleSetCompat) {
	t.Helper()
	content, err := singjson.Marshal(ruleSet)
	if err != nil {
		t.Fatalf("marshal rule-set json: %v", err)
	}
	if err := os.WriteFile(path, content, 0o644); err != nil {
		t.Fatalf("write rule-set json: %v", err)
	}
}
...
func assertLogicalRuleMatches(t *testing.T, expected, actual option.LogicalHeadlessRule) {
	...
	for i, expectedRule := range expected.Rules {
		...
		if expectedRule.Type != actualRule.Type {
			t.Fatalf("logical rule type mismatch: index=%d want=%s got=%s", i, expectedRule.Type, actualRule.Type)
		}
		switch expectedRule.Type {
		case C.RuleTypeDefault:
			assertDefaultRuleMatches(t, expectedRule.DefaultOptions, actualRule.DefaultOptions)
		case C.RuleTypeLogical:
			assertLogicalRuleMatches(t, expectedRule.LogicalOptions, actualRule.LogicalOptions)
		default:
			t.Fatalf("unexpected logical rule type: %s", expectedRule.Type)
		}
	}
}
